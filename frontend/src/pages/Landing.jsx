                                    ...
                                    return (
                                        <article key={service.id} className="tour-card bg-surface-container-lowest border border-outline-variant rounded-lg overflow-hidden flex flex-col h-full hover:border-primary transition-colors hover:shadow-md">
                                            ...
                                            <div className="p-6 flex-grow flex flex-col">
                                                <h3 className="font-headline-sm text-[20px] text-primary mb-2 leading-tight" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                                    {service.title}
                                                </h3>
                                                <p className="text-on-surface-variant flex-grow mb-4 text-sm" style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                                    {service.description?.length > 115 ? service.description.substring(0, 115) + '...' : service.description}
                                                </p>
                                                <div className="flex justify-between items-center mt-auto border-t border-outline-variant pt-4 h-[72px]">
                                                    <div className="flex flex-col justify-center h-full">
                                                        {service.old_price && parseFloat(service.old_price) > parseFloat(service.price) ? (
                                                            <>
                                                                <div className="flex items-center gap-2 mb-0.5">
                                                                    <span className="text-[12px] text-on-surface-variant line-through leading-none">S/ {parseFloat(service.old_price).toFixed(2)}</span>
                                                                    <span className="bg-error text-on-error text-[10px] font-bold px-1.5 py-0.5 rounded leading-none">-{Math.round((1 - service.price/service.old_price) * 100)}%</span>
                                                                </div>
                                                                <span className="text-primary font-bold text-[16px] whitespace-nowrap leading-none">S/ {parseFloat(service.price).toFixed(2)}</span>
                                                            </>
                                                        ) : (
                                                            <span className="text-primary font-bold text-[16px] whitespace-nowrap leading-none">Desde S/ {parseFloat(service.price).toFixed(2)}</span>
                                                        )}
                                                    </div>
